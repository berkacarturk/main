...
const int zEnaPin = 2;   // Enable pin (LOW = aktif, HIGH = devre dışı)
...
void setup() {
  ...
  pinMode(xEnaPin, INPUT_PULLUP);

  pinMode(yPulPin, OUTPUT);
  pinMode(yDirPin, OUTPUT);
  pinMode(yEnaPin, INPUT_PULLUP);

  pinMode(zPulPin, OUTPUT);
  pinMode(zDirPin, OUTPUT);
  pinMode(zEnaPin, INPUT_PULLUP);
...
void processSerialCommand(String command) {
  ...
  // Enable durumu sorgusu - XA YA ZA komutları
  if (command == "XA") {
    Serial.print("Xenabled=");
    Serial.println(xAxisEnabled ? "1" : "0");
    return;
  }
  else if (command == "YA") {
    Serial.print("Yenabled=");
    Serial.println(yAxisEnabled ? "1" : "0");
    return;
  }
  else if (command == "ZA") {
    Serial.print("Zenabled=");
